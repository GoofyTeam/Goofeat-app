...
export default function ResetPasswordScreen() {
  ...
  return (
    <AuthTemplate title="Réinitialiser le mot de passe" description="Saisissez votre nouveau mot de passe ci‑dessous">
      <View className="grid gap-6">
        ...
        <Button className="w-full" disabled={loading || !isValid} onPress={onSubmit}>Réinitialiser le mot de passe</Button>
        {!params.token && (
          <Text className="text-center text-sm">
            Vous n’avez pas de jeton ?{' '}
            <Link href="/forgot-password" asChild>
              <Text className="underline underline-offset-4">Demander un lien de réinitialisation</Text>
            </Link>
          </Text>
        )}
      </View>
